...
approved_at: '2026-03-02T10:36:37.173Z'
---
...
<FAQ>
  ...
  <FAQItem question="Does LobsterMail work with OpenClaw or MCP?">
    Yes. There's a ClawHub skill at [clawhub.ai](https://clawhub.ai/samuelchenardlovesboards/lobstermail-agent-email) that wires LobsterMail into OpenClaw with no manual SDK setup. MCP server support is also available — see the [MCP server guide](https://api.lobstermail.ai/v1/docs/guides/mcp-server).
  </FAQItem>
  ...
  <FAQItem question="What if I used AgentMail's webhook integration — is there an equivalent?">
    Yes. Call `inbox.setWebhook({ url: '...' })` to register a webhook on any inbox. The payload format differs from AgentMail's, so you'll need to update your handler to match LobsterMail's schema. The [webhooks guide](https://api.lobstermail.ai/v1/docs/guides/webhooks) has the full spec.
  </FAQItem>
