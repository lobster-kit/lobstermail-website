...
You passed a test token to a live endpoint or vice versa. Test [tokens](/glossary/tokens) start with `lm_sk_test_`, live tokens with `lm_sk_live_`. They're not interchangeable.
...
**Fix:** Retry with [exponential backoff](/glossary/exponential-backoff). Most agent frameworks have retry middleware; if yours doesn't, wrap SDK calls in a simple retry loop with a max attempt count.
...
**Fix:** Same as `LM_NETWORK_ERROR`, retry with backoff. If the issue persists, check the [LobsterMail docs](/docs) for known issues.
...
<FAQ>
  ...
  <FAQItem question="Is there a status page for API availability?">
    If you're seeing `LM_API_UNAVAILABLE` and can't find an obvious cause on your end, retry with exponential backoff. Persistent issues typically resolve within a few minutes during deployments.
  </FAQItem>
