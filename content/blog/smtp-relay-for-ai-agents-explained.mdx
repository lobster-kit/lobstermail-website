...
Rate limiting is the other concern. Most SMTP relay services enforce per-hour or per-day sending limits. An agent that hits the limit mid-workflow needs to queue remaining messages and resume later. Without this, the workflow just fails silently. If you're evaluating relay services for agent use, [agent email APIs compared: LobsterMail vs SendGrid vs Resend](/blog/lobstermail-vs-popular-alternatives) breaks down how each handles rate limits and error responses.
