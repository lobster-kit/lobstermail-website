...
<FAQ>
  ...
  <FAQItem question="Can I run multiple PydanticAI agents with separate inboxes under one account?">
    Yes. Each inbox is independent and scoped to its own ID. You can have a triage agent, a report-sending agent, and an invoice-parsing agent all running under a single LobsterMail account with separate addresses. The free tier gives you one inbox; Builder and above support up to 10.
  </FAQItem>
