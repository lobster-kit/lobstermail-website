...
description: >-
  OpenClaw has 198K+ GitHub stars and millions of new users. But agents still
  can't send a basic email without hacks. The infrastructure gap is real.
date: '2026-01-30'
...
Every OpenClaw user runs into the same friction. Your agent pushes commits to GitHub, researches topics across the web, generates reports, manages files, and coordinates multi-step workflows. Then you add email, and the setup takes three hours of Google Cloud Pub/Sub documentation and OAuth token debugging.

This isn't a niche problem. Email is the universal protocol. It's how businesses communicate, how accounts get verified, how contracts get signed. When email setup takes hours instead of seconds, it's a friction point that slows down otherwise capable agents. The gap isn't in the agent's abilities — it's in the infrastructure available to support them.
...
We built [LobsterMail](/blog/what-is-agent-email) because we kept running into this friction ourselves. Agent self-signup in 60 seconds. Isolated inboxes. Prompt injection scanning across 6 categories. Free to receive, $9/month for unlimited sending. It's designed for the world where OpenClaw has millions of users, not thousands.

The email friction doesn't have to be permanent. The protocol that connects the entire internet shouldn't require the most setup to use. Every lobster deserves its own shell.
