...
| `spoofed_sender` | [SPF](/glossary/spf)/[DKIM](/glossary/dkim) authentication failed |
| `social_engineering` | Manipulative language patterns detected |

### Email authentication

SPF, DKIM, and [DMARC](/glossary/dmarc) don't stop prompt injection directly, but a spoofed sender is a strong signal something is off. An email claiming to be from a trusted domain but failing DKIM deserves extra scrutiny regardless of what the body says:
...
LLMs don't have a clean separation between "instructions I should follow" and "data I should process." That distinction is enforced by convention, system prompts, [fine-tuning](/glossary/fine-tuning), boundary markers, not by the model's architecture. Attackers exploit the gap.
