...
approved_at: '2026-03-02T10:36:37.181Z'
---
...
The result is a `security` object attached to every email, with a numeric score from `0.0` (clean) to `1.0` (high-confidence attack), plus a `flags` array identifying what was detected:

```typescript
const email = await inbox.waitForEmail();

console.log(email.security.score);   // 0.83
console.log(email.security.flags);   // ['prompt_injection', 'spoofed_sender']
console.log(email.security.spf);     // 'fail'
console.log(email.security.dkim);    // 'fail'
```
...
inbox.on('email', async (email) => {
  ...
  const metadata = {
    injectionScore: email.security.score,
...
<FAQ>
  ...
  <FAQItem question="What is the injection risk score and what threshold triggers isInjectionRisk?">
    The score runs from `0.0` (clean) to `1.0` (high-confidence attack). The `isInjectionRisk` boolean is `true` when the score exceeds `0.5`. You can also read the raw score directly and apply your own threshold for finer control over borderline cases.
  </FAQItem>
  ...
  <FAQItem question="How do I handle emails with a borderline injection score without discarding them?">
    Read `email.security.score` and make a judgment call. For scores between `0.3` and `0.5`, a reasonable approach is to log the metadata, proceed with `safeBodyForLLM()` rather than the raw body, and flag the output for human review if the response will have consequences. The [OpenClaw agent email security guide](/blog/openclaw-agent-email-security) has a worked example with a full logging pattern.
  </FAQItem>

  <FAQItem question="What happens when SPF, DKIM, or DMARC fail on an inbound email?">
    A `spoofed_sender` flag is added to `email.security.flags`, and the individual authentication results are available as `email.security.spf`, `email.security.dkim`, and `email.security.dmarc`. A spoofed sender typically also elevates the overall injection score. Whether you discard or process the email is your decision.
  </FAQItem>
