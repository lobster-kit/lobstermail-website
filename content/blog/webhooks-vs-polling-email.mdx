...
**Verify signatures.** Every webhook from LobsterMail includes an [HMAC signature](/glossary/hmac-signature) in the `x-lobstermail-signature` header. Always verify it before processing the payload. This prevents attackers from sending spoofed emails to your endpoint.
