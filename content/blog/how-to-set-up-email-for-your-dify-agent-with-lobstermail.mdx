...
In your Dify workspace, go to **Tools → MCP Tools → Add MCP Server**. Enter the server URL and your authorization token. Full connection details are in [LobsterMail's MCP server guide](/docs/mcp-server).
...
<FAQ>
  ...
  <FAQItem question="What if two workflow runs try to create an inbox with the same name?">
    LobsterMail's smart [inbox provisioning](/glossary/inbox-provisioning) handles collisions automatically, trying variations like `my-agent1` or `m-agent` until it finds an available address. If you use the basic `create_inbox`, you get a random address with no collision risk at all.
  </FAQItem>
