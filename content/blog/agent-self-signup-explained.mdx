...
There's a moment in every agent project where a manual step interrupts the flow. You've built something autonomous — it can reason, plan, call APIs, write code — but when it needs an email address, you're the one opening a dashboard, creating an account, generating an API key, and pasting it into an `.env` file. That's a manual step the agent could handle itself.

That's the current state of agent email. The provisioning step still requires a human, even though it doesn't have to.
...
Five steps before the agent can do anything. Each one requires human availability. Each one introduces unnecessary friction — forgotten credentials, expired keys, misconfigured environments.

For a single agent, this is mildly annoying. For a multi-agent system where agents spawn sub-agents, or where an orchestrator provisions a fleet of specialists on demand? It's unnecessary friction at every step.
...
→ Inbox created: support-7b@lobstermail.ai
...
Without self-hatching, the orchestrator pauses. A human has to log into a console, create three inboxes, generate three sets of credentials, and wire them up. The workflow waits. The autonomy has a manual step in the middle.
...
<FAQ>
  ...
  <FAQItem question="What email address does a self-hatched inbox get?">
    On the free tier, the agent gets an address like `your-agent@lobstermail.ai`. On paid plans, you can use your own custom domain so the agent's address matches your brand.
  </FAQItem>
