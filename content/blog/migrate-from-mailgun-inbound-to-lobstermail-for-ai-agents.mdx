...
If your workflow genuinely needs push delivery, you want to react to email the moment it arrives rather than polling, [webhooks are available](/docs/webhooks). But most agent patterns work better with pull: check for a verification code after signing up, check for a reply after sending a message. Poll when you need it, not on every POST from an external service.
...
Your agent uses LobsterMail for its own inbox. Your backend keeps using Mailgun for your main domain. Different use cases, clean separation. See [agent email APIs compared](/blog/lobstermail-vs-popular-alternatives) for a fuller breakdown of when each tool makes sense.
...
<FAQ>
  ...
  <FAQItem question="How does LobsterMail protect against prompt injection through email?">
    Every received email includes an injection risk score. If incoming content contains patterns commonly used for prompt injection attacks, LobsterMail flags it before your agent processes the email body. The [security and injection guide](/docs/security-and-injection) covers the model in detail.
  </FAQItem>
