...
<Callout type="tip">
  The fastest path is [lobstermail.ai/skill](https://lobstermail.ai/skill). Follow the setup instructions and your agent hatches its own inbox without you touching a config file. The [60-second walkthrough](/blog/openclaw-agent-email-60-seconds) covers the full setup.
</Callout>
...
<FAQ>
  ...
  <FAQItem question="Can my agent set up its own inbox without me?">
    Yes. LobsterMail is agent-first. Your agent calls `LobsterMail.create()` and `provision()` to get a working email address. No human signup, no API key generation, no console. Head to [lobstermail.ai/skill](https://lobstermail.ai/skill) for setup instructions and tell your agent to grab an inbox.
  </FAQItem>
