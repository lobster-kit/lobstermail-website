...
|---|---|---|
| [Inbox provisioning](/glossary/inbox-provisioning) | Human creates account, configures DNS | Agent provisions its own inbox programmatically |
...
- **[Suppression list](/glossary/suppression-list) sync** so the agent never emails someone who opted out, even if a different system ingested the opt-out
...
<FAQ>
  ...
  <FAQItem question="How do AI agents handle dunning sequences for failed subscription payments?">
    The agent monitors payment [webhook](/glossary/webhook) events (like a declined card from Stripe), then initiates a multi-step dunning sequence: payment update email, retry with alternate payment method, grace period notification, and final account pause warning. Each step adapts based on the customer's response.
  </FAQItem>
  ...
  <FAQItem question="What compliance [guardrails](/glossary/guardrails) should be built into an AI agent managing renewal emails?">
