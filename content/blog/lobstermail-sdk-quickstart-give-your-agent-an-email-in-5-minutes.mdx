...
approved_at: '2026-03-02T10:36:37.178Z'
---
...
for (const email of emails) {
  ...
  console.log(email.injectionRisk); // 0.0 to 1.0
}
```
...
Full API reference is in the [docs](https://api.lobstermail.ai/v1/docs/guides/getting-started).
...
<FAQ>
  ...
  <FAQItem question="What does the injectionRisk field actually return?">
    A float between 0.0 and 1.0 representing the likelihood that an incoming email contains a prompt injection attempt. LobsterMail scores every email before your agent processes it. A score above 0.7 is generally worth flagging. How you act on it (filter, log, quarantine) is up to you.
  </FAQItem>
  ...
  <FAQItem question="Can I use my own domain for agent email addresses?">
    Yes. Custom domains let your agent send from `agent@yourdomain.com` instead of `@lobstermail.ai`. Setup is in the [docs](https://api.lobstermail.ai/v1/docs/guides/custom-domains) and available on the Builder plan.
  </FAQItem>

  <FAQItem question="How many inboxes can I create?">
    Free accounts support multiple inboxes sharing the 1,000 emails/month quota. The Builder plan includes up to 10 inboxes with higher send limits. See [how LobsterMail pricing works](/blog/lobstermail-pricing-explained) for full limits by tier.
  </FAQItem>
