...
<FAQ>
  ...
  <FAQItem question="Is injection risk scoring available via the API or only the SDK?">
    It's available through both. The SDK surfaces it as `email.injectionRisk` on each received message. The REST API includes the same metadata in the email response object. Either way, you get the score before deciding whether to pass the email content to your agent.
  </FAQItem>
